...
class FTTransformerModel(BaseModel):
    ...
    def fine_tune(
        ...
        """
        Further train the current model on new data.
        """
        if additional_epochs is None or additional_epochs <= 0:
            raise ValueError("`additional_epochs` must be a positive integer.")
    
        # ---- housekeeping ---
        start_epoch_num = len(self.train_loss)          # where we left off
        orig_total_epochs = self.epochs                 # remember for later
    
        # Re-initialise the LR scheduler so the cosine curve restarts
        if self.scheduler is not None:
            total_steps = additional_epochs * max(1, int(len(X_new_train) / self.batch_size))
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer, T_max=total_steps, eta_min=1e-5, last_epoch=-1
            )
    
        # Reset early-stopping counters *for this fine-tune only*
        self._early_stop_counter = 0
        self.best_val_r2_finetune = -float("inf")
        self.best_model_state_dict_finetune = None
    
        # Temporarily replace self.epochs with the number of *extra* epochs
        ...
        self._train_loop(
            X_new_train, y_new_train,
            X_new_val,   y_new_val,
            start_epoch=start_epoch_num           # <-- you’ll add this arg below
        )
    
        # Restore the original epoch setting
        self.epochs = orig_total_epochs
