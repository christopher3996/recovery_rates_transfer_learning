# FTTransformerModel (SOTA Rev) — drop-in replacement with same public interface
# Major upgrades:
# - Numerically stable MDN (log-sum-exp), optional learnable temperature
# - Optional bounded support via Beta-mixture head (default off)
# - Cleaner Transformer block: PreNorm + GEGLU FFN + stochastic depth
# - Fair fine_tune(): correct parameter groups (higher LR for head)
# - Utilities to get full predictive distributions and portfolio-level density
# - Backward compatible interface: train(), fine_tune(), predict(), select_best_model(), etc.
