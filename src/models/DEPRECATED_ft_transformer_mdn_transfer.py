# Version before major rework (07/2025)
# Included Regression Head for optional Ablation
...
class RegressionHead(nn.Module):
    ...
    def forward(self, x):
        return self.out(x), None  # keep same return format
...
class FTTransformerModel(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.criterion = (
            bounded_mdn_loss if use_mdn else nn.MSELoss()
        )
    ...
    def _train_loop(
        ...
        for global_ep in epoch_iter:
            ...
            for xb_s, xb_t, xc_s, xc_t, yb in train_loader:
                ...
                output = self.model(xb_s, xb_t, xc_s, xc_t)
                y_hat = output[0]
            ...
            # ─── validation ────────────────────────────────────────────────
            with torch.no_grad():
                output = self.model(Xv_num_sh, Xv_num_ta, Xv_cat_sh, Xv_cat_ta)
                y_pred_v_torch = output[0]
                
                if self.use_mdn:
                    a_v, m_v, s_v = output[1]
                    val_loss = self.criterion(a_v, m_v, s_v, Yv).item()
                    y_pred_v = (a_v * m_v).sum(1).cpu().numpy()
                else:
                    val_loss = self.criterion(y_pred_v_torch, Yv).item()
            ...
            if self.use_mdn:
                ...
                self.val_means.append(None)
                self.temperature_history.append(None)
            ...
            if self.early_stopping:
                ...
                if improved:
                    ...
                    if self._early_stop_counter >= self.patience:
                        ...
                        epoch_iter.close()
                        return ep + 1    # last epoch actually executed
        ...
        return self.epochs   # loop finished all epochs
    ...
    # ----------------------------------------------------------------------
    # put this in FTTransformerModel (replace your current reset_all_weights)
    # ----------------------------------------------------------------------
    def reset_weights(
        self,
        parts=("tokenizer", "backbone", "head"),   # choose any subset
        mode: str = "restore",                    # "restore" | "reinit"
    ):
        """
        Reset or restore selected parts of the network.
    
        Parameters
        ----------
        parts : iterable of {"tokenizer","backbone","head"}
            • "tokenizer"  → shared_*_emb + task_*_emb
            • "backbone"   → all Transformer encoder blocks
            • "head"       → MDNHead
        mode : {"restore","reinit"}
            • "restore"  – loads the very first parameters (cached once).
            • "reinit"   – redraws fresh random initialisation in place.
    
        Notes
        -----
        ✱ Call once *before* you freeze/unfreeze and rebuild the optimiser.
        ✱ After any reset a new optimiser is created automatically.
        """
        # mapping sub-strings → block selector
        name_filters = {
            "tokenizer": ("shared_num_emb", "shared_cat_emb",
                          "task_num_emb",   "task_cat_emb"),
            "backbone":  ("blocks.",),          # catches every encoder block
            "head":      ("head.",),
        }
    
        # ------------------ collect parameter names --------------------------
        wanted = set(parts)
        unknown = wanted - name_filters.keys()
        if unknown:
            raise ValueError(f"Unknown part(s): {unknown}")
    
        def _param_belongs(name):
            return any(name.startswith(prefix) for p in wanted
                       for prefix in name_filters[p])
    
        # ------------------ snapshot on first call ---------------------------
        if not hasattr(self, "_init_state_dict"):
            print("Has not _init_state_dict")
            self._init_state_dict = copy.deepcopy(self.model.state_dict())
        ...
        if mode == "restore":
            ...
            for k, v in self._init_state_dict.items():
                if _param_belongs(k):
                    current[k].copy_(v)            # in-place restore
            self.model.load_state_dict(current)
    
        elif mode == "reinit":
            for n, p in self.model.named_parameters():
                if _param_belongs(n):
                    _init_weights(getattr(self.model, n.split('.')[0]))
        else:
            raise ValueError("mode must be 'restore' or 'reinit'")
    
        # ------------------ new optimiser (momentum buffers invalid) ---------
        self._rebuild_optimizer(lr=self.optimizer.defaults["lr"],
    ...
    def fine_tune(
        ...
        #=================
        #NEW 
        #self.reset_all_weights(mode="restore")   # or "reinit"
        #self.reset_weights(parts=("tokenizer", "backbone", "head"), mode="reinit")
        #self.reset_weights(parts=("head",), mode="reinit")

        self.model.rebuild_task_banks(len(self.task_num_cols), cat_cards_task)


        # ---- inside fine_tune() right after task-bank rebuild ----
        self.reset_weights(parts=('head',), mode='reinit')
        self._rebuild_optimizer(lr, weight_decay)       # keep lr; head will get ×5 below
        for g in self.optimizer.param_groups:           # last param-group is the head
            g['lr'] *= 5

        # # NEW
        # self.model.head = MDNHead(
        #     self.model.d_token,
        #     n_components=2,         # keep your hyper-params
        #     hidden_size=24,
        #     temperature=2.5            # re-start from prior
        # ).to(self.device)
    ...
    def get_epoch_metrics(self):
        """Return split metric lists for pre-train and fine-tune."""
        metrics = {}
    
        # ------------------ pre-train ------------------
        n_pre = getattr(self, "max_train_epoch", 0) or 0   # pure count
        if n_pre:
